...
<div class="scBoard">
	...
	<div style="text-align:center;" class="scBoardtext">
		...
		<h5><b>YELLOW BRIDGE 서비스 Q&A</b></h5>
	</div>
				...
				<table id="example" class="display" style="width:100%">
			        ...
			        <tbody id = "listArea">		
			        	<c:forEach var="qna" items="${ qnalist }">
							<tr>
								<td align="center">${ qna.qId }</td>
								<td align="center">
										<c:url var="qnaDetail" value="qnadetail.sc">
			    ...
			    <div align="center"id="buttonTab">
					...
					<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
						...
						<c:if test="${ p ne pi.currentPage }">
							<c:url var="pagination" value="qna.sc">
					...
					<c:if test="${ pi.currentPage < pi.maxPage }">
						<c:url var="after" value="qnaList.sc">
							<c:param name="page" value="${ pi.currentPage + 1 }"/>
						</c:url> 
						<a href="${ after }">[다음]</a>
						</c:if>
					<div class="btnDiv">
					<button type="button" class="btn btn-primary" id="writeBtn" onclick="location.href='<%= request.getContextPath() %>/qnainsertform.sc'">글쓰기</button>
				</div>
			</div>
		</div>
	</div>
