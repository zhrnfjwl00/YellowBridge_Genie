...
<body>
	<div class="container">
		...
		<form name="readForm" role="form" method="post">
			...
			<input type="hidden" id="qTitle" name="qTitle" value="${board.qTitle}"> 
			<input type="hidden" id="qWriter" name="qWriter" value="${board.qWriter}">
			<input type="hidden" id="id" name="id" value="${loginId}">
			<input type="hidden" id="qContent" name="qContent" value="${board.qContent}"> 
			<input type="hidden" id="qCategory" name="qCategory" value="${board.qCategory}"> 
			<input type="hidden" id="qCount" name="qCount" value="${board.qCount}"> 
			<input type="hidden" id="qCreateDate" name="qCreateDate" value="${board.qCreateDate}"> 
			<%-- <input type="hidden" id="id" name="id" value="${ sessionScope.loginUser }">  --%>
			<input type="hidden" id="page" name="page" value="${page}"> 
		</form> 

		<c:url var="qnaupdateForm" value="qnaupdateForm.sc">
			<c:param name="bNo" value="${ board.qId }" />
			<c:param name="page" value="${ page }" />
		</c:url>

		<c:url var="qnadelete" value="qnadelete.sc">
			<c:param name="bNo" value="${ board.qId }" />
		</c:url>
		...
		<section id="container">
			<div class="form-group">
				<table>
					<tr>
						<th id="title">${ board.qTitle }</th>
						<th>
							<c:if test="${loginUser.grade  == 1}">
							<button class="delete_btn btn btn-danger" id="deleteBtn"
								onclick="location.href='${qnadelete}'">삭제</button>
							</c:if>
							
							<c:if test="${ sessionScope.Id  == {board.qWriter} }">
							<button class="delete_btn btn btn-danger" id="deleteBtn"
								onclick="location.href='${qnadelete}'">삭제</button>
							</c:if>
					...
					<tr>
						<td><b>작성일</b>&nbsp;&nbsp;${ board.qCreateDate } &nbsp;&nbsp;</td>
						<td>| <b>조회수</b> &nbsp;&nbsp;${ board.qCount }
						</td>
					</tr>
				</table>
			...
			<div class="form-group">
				...
				<textarea id="content" name="content" class="form-control" cols="60"
					rows="25" readonly="readonly" style="resize: none">${board.qContent}</textarea>
			</div>
			
			<c:url var="qnalist" value="qnaList.sc">
					<c:param name="page" value="${ page }"/>
			</c:url>
			<div align="center">
				<button type="button" class="btn btn-primary" id="writeBtn"
					onclick="location.href='<%=request.getContextPath()%>/qna.sc'">목록</button>
			</div>
